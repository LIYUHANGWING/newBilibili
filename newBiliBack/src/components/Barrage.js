class Barrage{
    ...
    initTop(highwayAmount){
        ...
        // 如果传入传入的highwayAmount > 最大轨道数，那么就要让highwayAmount = maxHighwayAmount
    ...
    drawBarrage(){
        ...
        this.barrageList.forEach((val)=>{
            this.context.fillStyle = val.color // 设置弹幕的颜色

            this.context.fillText(`${val.value}`, val.left, val.top)
            val.occupation && val.left + val.width <= this.canvasWidth ? this.consumeTest(val): ''
            val.left -= 2
        })
        if(this.clickStop)
